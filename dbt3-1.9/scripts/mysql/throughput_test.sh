...
while getopts "ef:jn:o:p:s:t:yx" opt; do
	case $opt in
...
# Start the streams.
${SHELL} $DBSCRIPTDIR/throughput_stream_wrapper.sh -f $scale_factor -t $TAG -n $num_stream $TEST_ARGS $NO_EXPLAIN -o $OUTPUT_DIR -s ${SEED_FILE} || exit 1
