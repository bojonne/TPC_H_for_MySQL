...
while getopts "f:g:ilmo:p:tyvxI:" opt; do
	case $opt in
	f)
		SF=$OPTARG
		;;
...
${SHELL} $DBSCRIPTDIR/create_tables.sh ${DB_ENGINE} ${FIXED_CHAR} ${TABLESPACE_FLAG} || exit 1
...
if [ ${ONLY_LOAD} -eq 0 ]; then
	...
	if [ "x${OPT_IDX_EXE}" != "x" ]; then
		echo "Add optional indexes."
		${SHELL} ${OPT_IDX_EXE} || exit 1
	fi
	date
fi
