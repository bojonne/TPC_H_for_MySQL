...
while getopts "ef:jp:o:s:t:yx" opt; do
	case $opt in
		e) EXPLAIN_ANALYZE=1
			;;
...
if [ $NO_EXPLAIN -eq 0 ]; then
	...
	${SHELL} $DBSCRIPTDIR/record_start.sh -n "PERF${TAG}.POWER.EXPLAIN" || exit 1
	...
	do
		if [ $i -ne 15 -a $i -ne 22 ]; then
			${SHELL} $DBSCRIPTDIR/record_start.sh -n "PERF${TAG}.POWER.EXPLAIN.Q${i}" || exit 1
			${SHELL} $DBSCRIPTDIR/get_query_plan.sh $SCALE_FACTOR $i $PLANDIR/power_query$i.txt $RUNDIR $SEED_FILE
			${SHELL} $DBSCRIPTDIR/record_end.sh -n "PERF${TAG}.POWER.EXPLAIN.Q${i}" || exit 1
		fi
		let "i=$i+1"
	done
	${SHELL} $DBSCRIPTDIR/record_end.sh -n "PERF${TAG}.POWER.EXPLAIN" || exit 1	
fi
...
${SHELL} $DBSTATS $DBSTATS_FLAG $SID $OUTPUT_DIR &
