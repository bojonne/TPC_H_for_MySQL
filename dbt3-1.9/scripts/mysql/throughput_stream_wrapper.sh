...
EXPLAIN_ANALYZE=0
NO_EXPLAIN=0
while getopts "ef:jn:o:s:t:" opt; do
	case $opt in
	e) EXPLAIN_ANALYZE=1
		;;
...
do
	bash $DBSCRIPTDIR/run_throughput_stream.sh $scale_factor $TAG $i $EXPLAIN_ANALYZE $OUTPUT_DIR $SEED_FILE $NO_EXPLAIN > $RUNDIR/thruput_qs$i 2>&1 || exit 1 &
