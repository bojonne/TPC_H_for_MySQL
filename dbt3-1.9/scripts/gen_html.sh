...
if [ $# -ne 1 ]; then
	echo "usage: gen_html.sh [-x] <results directory>"
fi
...
OSVER=`uname -s -r`
DBVER=`${SHELL} $SRCDIR/scripts/${DBDIR}/get_version.sh`
...
if [ "mysql" = "mysql" ]; then
	LOAD=`grep LOAD ${DIR}/q_time.out | awk '{ print $12 }'`
else
...
gen_results()
{
	...
	if [ "mysql" = "pgsql" -a $DBSTATS_FLAG -eq 1 ]; then
	...
	if [ "mysql" = "mysql" -a $DBSTATS_FLAG -eq 1 ]; then
		echo "<a href=\"db/innodb_status.txt\">InnoDB Status</a><br />" >> ${SUBDIR}/index.html
		echo "<a href=\"db/global_status.txt\">Global Status</a><br />" >> ${SUBDIR}/index.html
	fi
