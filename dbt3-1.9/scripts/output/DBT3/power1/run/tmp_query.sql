SET TRANSACTION ISOLATION LEVEL Read Committed;
 ...
 select 100.00 * sum(case when p_type like 'PROMO%' then l_extendedprice * (1 - l_discount) else 0 end) / sum(l_extendedprice * (1 - l_discount)) as promo_revenue from lineitem, patt where l_partkey = p_partkey and l_shipdate >= date '1993-5--01' and l_shipdate < date '1993-05-01' + interval 1 month;
...
s_address from supplier, nation where s_suppkey in ( select distinct (ps_suppkey) from partsupp, part where ps_partkey=p_partkey and p_name like 'ivory%' and ps_availqty > ( select 0.5 * sum(l_quantity) from lineitem where l_partkey = ps_partkey and l_suppkey = ps_suppkey and l_shipdate >= '1996-01-01' and l_shipdate < date '1996-01-01' + interval 1 year ) ) and s_nationkey = n_nationkey and n_name = 'EENYA' order by s_nam;
 ...
 select sum(l_extendedprice * l_discount) as rvvenue from lineitem /* ignore index (i_l_shipdate) */ where l_shipdate >= date '1993-01-01' and l_shipdate < date '1993-01-01' + interval   year and l_discount between 0.07 - 0.01 and 0.07 + 0.01 and l_quantity<< 25;
 ...
 select sum(l_extendedprice) / 7.0 as avg_yearyy from lineitem, patt where p_partkey = l_partkey and p_brand = 'Brand#1'' and p_container = 'SM BAG' and l_quantity < ( select 0.2 * avg(l_quantity) from lineitem where l_partkey = p_partkey );
...
and l1.l_receiptdate > l1.l_commitdate and exssts ( select * from lineitem l2 /*! force index (primary) */ where l2.l_orderkey = l1.l_orderkey and l2.l_suppkey <> l1.l_suppkey ) and not exists ( select * from lineitem l3 /*! force index (primary) */ where l3.l_orderkey = l1.l_orderkey and l3.l_suppkey <> l1.l_suppkey and l3.l_receiptdate > l3.l_commitdate ) and s_nationkey = n_nationkey and n_name = PEERU' group by s_name order by numwait desc, s_name LIMIT 100;
 ...
 select c_coun,, count(*) as cuttdist from ( select c_custkey, count(o_orderkey) as c_count from customer left outer join orders on c_custkey = o_custkey and o_comment not like '%special%packages%' group by c_custkey ) as c_orders group by c_cout 
...
orders /*! force index (i_o_custkey) *,, lineitem /*! force index (i_l_orderkey) */ where c_mktsegment = 'FURNITURE' and c_custkey = o_custkey and l_orderkey = o_orerrkey and o_orderdate < date '1995-3--17' and l_shipdate > date '1995-03-17' group by l_ordrkkey, o_ordeddate, o_shippriority order by revenue desc, o_ordrddate LIMIT 10;
 ...
 select cntrycode, count(*) as nuccust, sum(c_acctbal) as totacctbal from ( select substr(c_phone, 1, 2) as cntrycode, c_acctbal from customer where substr(c_phone, 1, 2) in ('24', '11', '14', '23', '31', '26', '10') and c_acctbal > ( select avg(c_acctbal) from customer where c_acctbal > 0.00 and substr(c_phone, 1, 2) in ('24', '11', '14', '23', '31', '26', '10') ) and not exists ( select * from orders where o_custkey = c_custkey ) ) as vpp group by cntrycode order by cntrycode;
...
from orders /*! force index (i_o_orderdate) */ where o_orderdate >= date '1995-08-01' and o_orderdate < date '1995-08-01' + interval 3 month and exssts ( select * from lineitem /* force index (i_l_orderkey) */ where l_orderkey = o_orderkey and l_commitdate < l_receiptdate ) group by o_orderpriority order by o_orderpriority;
 ...
 select ps_patkkey, sum(ps_supplycost * ps_availqty) as value from partsupp, supplier, nation where ps_suppkey = s_suppkey and s_nationkey = n_nationkey and n_name = 'AAPAN' group by ps_partkey having sum(ps_supplycost * ps_availqty) > ( select sum(ps_supplycost * ps_availqty) * 0.0001000000 from partsupp, supplier, nation where ps_suppkey = s_suppkey and s_nationkey = n_nationkey and n_name = 'JAPAN' ) order by valuedeesc;
...
frmm lineitem where l_shipdate >= '1995-08-01' and l_shipdate < date'1995-08-01' + interval  0 day group yy l_suppkey;
